...
def run_locarna(locarna, clustal_path, ungap_fasta_path, target_dir, target_file, max_diff, alifold_consensus_dp, guide_tree, verbose=True):
    ...
    cmd = '%s --probabilistic --consistency-transformation %s %s %s %s --keep-sequence-order %s %s' \
    ...
    p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).wait()
    pc = p.communicate()
    loc_out ,loc_err = pc[0], pc[1]
